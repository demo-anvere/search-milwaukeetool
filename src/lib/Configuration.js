...
var Configuration = /*#__PURE__*/ (function () {
	function Configuration() {
		...
		this.server.cacheSearchResultsForSeconds =
			this.server.cacheSearchResultsForSeconds == null
				? 2 * 60
				: this.server.cacheSearchResultsForSeconds
		...
		Object.keys(this.collectionSpecificSearchParameters).forEach(function (
			collection
		) {
			var overrideHighlightFullFields =
				_this.collectionSpecificSearchParameters[collection]
					.highlightFullFields ||
				_this.collectionSpecificSearchParameters[collection].queryBy

			if (overrideHighlightFullFields) {
				_this.collectionSpecificSearchParameters[
					collection
